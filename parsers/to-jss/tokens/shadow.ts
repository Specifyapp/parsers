  ...
  toJss({ shadowFormat = 'string', colorFormat = 'rgb' }: FormatTokenType) {
    return this.value.reduce((acc, shadow, idx, arr) => {
      ...
      if (shadowFormat === 'object')
        return before + prepend + JSON.stringify({
          x: x.measure,
          y: y.measure,
          blur: bl.measure,
          spread: 0, // TODO add spread value to api results
          color: tinycolor(color.value).toString(colorFormat),
          inset: innerText,
        }) + after;
      else if (shadowFormat === 'array')
        return `${before}${prepend}${innerText}${x.measure}, ${y.measure}, ${bl.measure}, '${tinycolor(color.value).toString(colorFormat)}'${after}`;
      else
        return `${before}${prepend}${innerText}${x.measure}${x.unit} ${y.measure}${y.unit} ${bl.measure}${bl.unit} ${tinycolor(color.value).toString(colorFormat)}${after}`;
    }, '');
  }
}
