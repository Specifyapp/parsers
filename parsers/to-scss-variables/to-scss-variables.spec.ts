...
describe('To scss', () => {
  ...
  it.only('Get tokens - apply parsers - all tokens', async () => {
    ...
    const result = await toScss(seeds().tokens, options, libs);
    console.log(result);
    // expect(result.includes('$heuristic-cross-platform-quantify: rgba(51, 15, 99, 0.6);')).toBe(true);
    // expect(
    //   result.includes(
    //     '$open-system-markets-hard-drive: "https://specifyapp.com/_nuxt/img/881a6b6.webp";',
    //   ),
    // ).toBe(true);
    // expect(result.includes('$invoice-system-worthy-payment: 7px solid rgba(199, 48, 37, 0.93);')).toBe(true);
    ...
    //   ),
    // ).toBe(true);
    // expect(result.includes('$kids-iowa-ergonomic: Allan-Bold 5pt rgba(86, 225, 86, 0.52);')).toBe(true);
    // expect(
    //   result.includes(
