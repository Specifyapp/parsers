...
describe('Optimize vector', () => {
  it('Get tokens - apply parsers', async done => {
    ...
    expect(result.find(({ name }) => name === 'from-sketch')!.value.content).toEqual(
    ...
    expect(result.find(({ name }) => name === 'from-figma')!.value.content).toEqual(
    ...
    expect(result.find(({ name }) => name === 'from-figma')!.value).toEqual(
