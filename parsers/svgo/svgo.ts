...
describe('Optimize vector', () => {
  it('Get tokens - apply parsers', async done => {
    ...
    expect(
      (result.find(({ name }) => name === 'alert-circle.svg') as DownloadableFile)!.value.content,
    ).toEqual(
    ...
    expect(
      (result.find(({ name }) => name === 'from-sketch.svg') as DownloadableFile)!.value.content,
    ).toEqual(
    ...
    expect(
      (result.find(({ name }) => name === 'from-figma.svg') as DownloadableFile)!.value.content,
    ).toEqual(
